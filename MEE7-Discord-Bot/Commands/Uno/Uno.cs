    ...
    public class Uno : Command
    {
        ...
        private static SKBitmap GetPicture(UnoColor c, UnoCardType t)
        {
            ...
            if (IsNumber(t))
            {
                ...
                return CardsTexture.CropImage(SkiaSharpExtensions.CreateRectFromXYWH(X, Y, CutOutWidth, CutOutHeight), false);
            }
            else
            {
                if (t == UnoCardType.changecolor)
                    return CardsTexture.CropImage(SkiaSharpExtensions.CreateRectFromXYWH((int)(9 * cardWidth), (int)(0 * cardHeight), CutOutWidth, CutOutHeight), false);
                else if (t == UnoCardType.plus4)
                    return CardsTexture.CropImage(SkiaSharpExtensions.CreateRectFromXYWH((int)(9 * cardWidth), (int)(2 * cardHeight), CutOutWidth, CutOutHeight), false);
                else if (t == UnoCardType.skip)
                {
                    int XNumber = c == UnoColor.red ? 0 : (c == UnoColor.yellow ? 1 : (c == UnoColor.blue ? 2 : (c == UnoColor.green ? 3 : -1)));
                    return CardsTexture.CropImage(SkiaSharpExtensions.CreateRectFromXYWH((int)(XNumber * cardWidth), (int)(4 * cardHeight), CutOutWidth, CutOutHeight), false);
                }
                ...
                {
                    if (c == UnoColor.red)
                        return CardsTexture.CropImage(SkiaSharpExtensions.CreateRectFromXYWH((int)(8 * cardWidth), (int)(4 * cardHeight), CutOutWidth, CutOutHeight), false);
                    else if (c == UnoColor.yellow)
                        return CardsTexture.CropImage(SkiaSharpExtensions.CreateRectFromXYWH((int)(9 * cardWidth), (int)(4 * cardHeight), CutOutWidth, CutOutHeight), false);
                    else if (c == UnoColor.blue)
                        return CardsTexture.CropImage(SkiaSharpExtensions.CreateRectFromXYWH((int)(0 * cardWidth), (int)(5 * cardHeight), CutOutWidth, CutOutHeight), false);
                    else if (c == UnoColor.green)
                        return CardsTexture.CropImage(SkiaSharpExtensions.CreateRectFromXYWH((int)(1 * cardWidth), (int)(5 * cardHeight), CutOutWidth, CutOutHeight), false);
                }
            }
            return null;
        }
