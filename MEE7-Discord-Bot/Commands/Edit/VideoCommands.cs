...
namespace MEE7.Commands.Edit
{
    class VideoCommands : EditCommandProvider
    {
        ...
        public Video BakaMitai(SKBitmap bmp, IMessage m)
        {
            ...
            lock (firstOrderModelLock)
            {
                ...
                SKBitmap resized = bmp.Stretch(256, 256);


                if (File.Exists(picFile))
                    File.Delete(picFile);

                var imgData = resized.Encode(SKEncodedImageFormat.Png, 100);
                using var fs = new FileStream(picFile, FileMode.Create);
                imgData.SaveTo(fs);
